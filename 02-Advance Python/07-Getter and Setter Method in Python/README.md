...
- `_name` and `_age` are private attributes of the Student class. 
- Accessor methods (`get_name()` and `get_age()`) are used to retrieve their values, while mutator methods (`set_name()` and `set_age()`) are used to update their values. 
- The mutator method `set_age()` includes a condition to ensure that the age is non-negative.
