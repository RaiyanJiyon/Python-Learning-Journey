...
'''In the first example, numpy.where() returns a tuple containing the indices where the condition arr > 2 is True. In this case, elements at indices 2, 3, and 4 satisfy the condition.

In the second example, numpy.where() is used with all three arguments. The first argument is the condition, the second argument specifies the value to be used where the condition is True, and the third argument specifies the value to be used where the condition is False. In this case, 'greater' is returned where the condition is True, and 'lesser or equal' is returned where the condition is False.'''